...
public class ReviewsForBookListContoller extends Main_con{
	volatile static boolean flag=true;
	private Review tmpReview;
	private DateFormat format;
	private ArrayList<Review> reviews;
	public ReviewsForBookListContoller(String bookId){
	super();
	reviews=new ArrayList<Review>();
	tmpReview= new Review();
	format = new SimpleDateFormat("yyyy-MM-dd");
	ArrayList<String> ReviewList = new ArrayList<String>();
	ReviewList.add("select"); 									//type of the query
	ReviewList.add("reviews");    									//from
	ReviewList.add("check_status=1 and book_id="+bookId);  // where 
    client.handleMessageFromClientUI(ReviewList);
    while(flag)
    {
